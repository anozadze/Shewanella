...
niter = 1*10^3; %number of iterations
...
% particles should fit inside walls! While also being properly distributed
...
seed = 1; % seed for calculating the turning probability due to chemotaxis
%%
...
for ii = 1:niter
    ...
    for pp = 1:npart
        ...
        while flag == 1
            ...
            % calculate the local derivative of the concentration gradient
            dc = -c_local/x0;

            % recalculate the new direction of particle pp
            % while considering the turning probability due to chemotaxis
            up_new = u(pp);
            up_new = taxis(c_local,c0,dc,up_new,seed);
            seed = seed+1;
            ...
            % update u_new (directions) for particle pp
            u_new(pp) = up_new;
    ...
    if mod(ii,nplot) == 0
        ...
        %%v here, not v_c!!
        harryplotter(type,wall,pd_max,x,v,vmax,vrange,u,sigma_pot,npart,c0,x0)
        %%
